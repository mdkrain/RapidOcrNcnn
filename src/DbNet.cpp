...
DbNet::~DbNet() { net.clear(); }

void DbNet::setNumThread(int numOfThread) { numThread = numOfThread; }
...
std::vector<TextBox> findRsBoxes(const cv::Mat &predMat,
                                 const cv::Mat &dilateMat, ScaleParam &s,
  ...
  int numContours =
      contours.size() >= maxCandidates ? maxCandidates : contours.size();
...
std::vector<TextBox> DbNet::getTextBoxes(cv::Mat &src, ScaleParam &s,
  ...
  memcpy(fMapMat.data, (float *)out.data,
